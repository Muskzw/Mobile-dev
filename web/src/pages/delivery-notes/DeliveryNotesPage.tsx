...
export default function DeliveryNotesPage() {
    ...
    const { data: deliveryNotes, isLoading } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get("/documents?type=delivery_note");
            return res.data;
        },
    });

    const filteredNotes = deliveryNotes?.filter((dn: any) =>
        statusFilter === "all" ? true : (dn.status || 'draft').toLowerCase() === statusFilter
    );
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Track goods and services delivery</p>
                    </div>
                    <button
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-between glass-card p-2 rounded-xl">
                    <div className="flex p-1 bg-muted rounded-lg">
                        {['all', 'draft', 'sent', 'delivered'].map((status) => (
                            <button
                                key={status}
                ...
                {isLoading ? (
                        ...
                        {filteredNotes.map((note: any) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(note.status || 'draft')}`}>
                                        {note.status || 'Draft'}
                                    </span>
                                </div>

                                <h3 className="font-bold text-lg mb-1">{note.client_name || "Untitled Client"}</h3>
                                <p className="text-sm text-muted-foreground mb-4">#{note.document_number}</p>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-border">
                                    ...
                                    <div className="font-bold">
                                        {note.currency} {Number(note.total).toFixed(2)}
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="text-center py-20 glass-card rounded-xl border border-dashed border-border">
                        ...
                        <h3 className="text-lg font-semibold mb-1">No delivery notes found</h3>
                        <p className="text-muted-foreground mb-6">Get started by creating your first delivery note.</p>
                        <button
                            onClick={() => navigate("/delivery-notes/new")}
