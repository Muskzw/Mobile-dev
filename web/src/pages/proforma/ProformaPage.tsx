...
export default function ProformaPage() {
    ...
    const { data: proformas, isLoading } = useQuery({
        queryKey: ["proformas"],
        queryFn: async () => {
            const res = await api.get("/documents?type=proforma");
            return res.data;
        },
    });

    const filteredProformas = proformas?.filter((p: any) =>
        statusFilter === "all" ? true : (p.status || 'draft').toLowerCase() === statusFilter
    );
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Manage proforma invoices for advance quotes</p>
                    </div>
                    <button
                ...
                {isLoading ? (
                    <div className="text-center py-20">
                        ...
                        <p className="text-muted-foreground">Loading proformas...</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredProformas.map((proforma: any) => (
                            <div
                                key={proforma.id}
                                onClick={() => navigate(`/proforma/${proforma.id}`)}
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(proforma.status || 'draft')}`}>
                                        {proforma.status || 'Draft'}
                                    </span>
                                </div>

                                <h3 className="font-bold text-lg mb-1">{proforma.client_name || "Untitled Client"}</h3>
                                <p className="text-sm text-muted-foreground mb-4">#{proforma.document_number}</p>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-border">
                                    ...
                                    <div className="font-bold">
                                        {proforma.currency} {Number(proforma.total).toFixed(2)}
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="text-center py-20 glass-card rounded-xl border border-dashed border-border">
                        ...
                        <h3 className="text-lg font-semibold mb-1">No proforma invoices found</h3>
                        <p className="text-muted-foreground mb-6">Get started by creating your first proforma invoice.</p>
                        <button
                            onClick={() => navigate("/proforma/new")}
