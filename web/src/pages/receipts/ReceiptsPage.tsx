...
export default function ReceiptsPage() {
    ...
    const { data: receipts, isLoading } = useQuery({
        queryKey: ["receipts"],
        queryFn: async () => {
            const res = await api.get("/documents?type=receipt");
            return res.data;
        },
    });

    const filteredReceipts = receipts?.filter((r: any) =>
        statusFilter === "all" ? true : (r.status || 'draft').toLowerCase() === statusFilter
    );
            ...
            <div className="max-w-7xl mx-auto space-y-8 pb-20">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-1">Manage payment receipts and confirmations</p>
                    </div>
                    <button
                        ...
                        {filteredReceipts.map((receipt: any) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(receipt.status || 'draft')}`}>
                                        {receipt.status || 'Draft'}
                                    </span>
                                </div>

                                <h3 className="font-bold text-lg mb-1">{receipt.client_name || "Untitled Client"}</h3>
                                <p className="text-sm text-muted-foreground mb-4">#{receipt.document_number}</p>
